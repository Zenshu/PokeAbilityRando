...
Advanced Filters: Filter rolls by specific generations (Gen 3 onwards) or choose "All Gens", with an optional toggle to dynamically strip out abilities that only function in double or multi-battles (DOUBLES_ONLY_ABILITIES).
...
Wildcard Support: Seamlessly flags designated wildcard tokens during generation for custom rando rulesets.

Smart Auto-Updater: Automatically checks for GitHub releases in the background, validating binary payloads and using platform-native update routines (update.bat for Windows and update.sh for Unix/macOS) to handle file locking safely.
